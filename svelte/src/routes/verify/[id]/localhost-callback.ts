...
export function addLocalhostCallbackState(callbackUrl: string, callbackSecret: string): string {
  ...
  url.searchParams.set('state', callbackSecret);
  return url.href;
}

/** Recovers a verified callback without having the registry reflect callback state. */
export async function recoverLocalhostCallbackUrl(
  ...
  return addLocalhostCallbackState(body.localhost_callback_url, callbackSecret);
}

/** Delivers an MFA OTP only to the locally constructed Cargo callback shape. */
