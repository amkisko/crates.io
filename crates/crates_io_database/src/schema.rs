...
diesel::table! {
    ...
    api_mfa_challenges (id) {
        /// API token that created the challenge, if any
...
diesel::table! {
    ...
    /// Short-lived cargo login ceremonies: URL → browser scopes → one-time poll
    cli_login_sessions (id) {
        /// API token minted on approve, if any
        api_token_id -> Nullable<Int4>,
        ...
        expires_at -> Timestamptz,
        /// Public opaque session id used in login URLs
        id -> Varchar,
        /// Last successful poll request time (pace rapid polling)
...
diesel::table! {
    ...
    api_mfa_grants (id) {
        ...
        crate_name -> Nullable<Varchar>,
        /// Date and time when the grant will expire
        expires_at -> Timestamptz,
        /// Unique identifier of the `api_mfa_grants` row
        id -> Int8,
        /// When set, grant only covers this operation; NULL means any operation
        operation -> Nullable<Varchar>,
        /// User that received the grant
        user_id -> Int4,
    }
}
...
diesel::table! {
    ...
    webauthn_ceremony_states (id) {
        ...
        created_at -> Timestamptz,
        /// Date and time when the ceremony state expires
        expires_at -> Timestamptz,
        /// Unique identifier of the `webauthn_ceremony_states` row
        id -> Int8,
        /// Ceremony kind: `registration` or `authentication`
        kind -> Varchar,
        /// Serialized PasskeyRegistration or PasskeyAuthentication state
        state_json -> Jsonb,
        /// User performing the ceremony
        user_id -> Int4,
    }
}
...
diesel::table! {
    ...
    webauthn_credentials (id) {
        /// Date and time when the credential was registered
        created_at -> Timestamptz,
        /// Raw WebAuthn credential ID (unique across users)
        credential_id -> Bytea,
        /// Unique identifier of the `webauthn_credentials` row
        id -> Int8,
        /// Date and time when the credential was last used
        last_used_at -> Nullable<Timestamptz>,
