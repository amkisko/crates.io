...
const schema = v.pipe(
  v.object({
    ...
    /** Plaintext for MSW only; real API stores a hash and returns the secret once from start. */
    pollSecret: v.optional(v.string()),
  ...
  v.transform(function (input) {
    ...
    let pollSecret = input.pollSecret ?? `pollsecret_${counter.toString(36).padStart(24, '0')}`;
    return { ...input, id, expiresAt, createdAt, confirmationCode, pollSecret };
  }),
);
