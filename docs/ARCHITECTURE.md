...
Users can optionally enable API MFA. Cargo publish, yank, and owner changes then
use registry mutation authorization; protected website actions use a recent
browser-session passkey authorization. See [`API-MFA.md`](API-MFA.md).
Browser-assisted `cargo login` (link → scopes → one-time poll) is documented
in [`CLI-LOGIN.md`](CLI-LOGIN.md).
