...
- With API MFA: publish, yank, and owner changes from the site require an active grant (Authorize for 15 minutes); minting tokens still uses the session; registering an additional passkey or disabling API MFA requires a passkey assertion
...
WebAuthn passkeys are the supported mechanism today.
...
3. API responds `403` with structured fields:
   ...
   - `verification_url` — browser page for passkey verification (`/webauthn-verify/{operation_id}`; no sign-in)
   - `poll_url` — `GET /api/v1/me/api_mfa/challenges/{operation_id}`
...
Stock `cargo` does not yet parse `api_mfa_required` or poll for acknowledgment. Until cargo grows that support:
